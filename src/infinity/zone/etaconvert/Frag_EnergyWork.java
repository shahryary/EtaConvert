...
import infinity.zone.convertors.EnergyWork;
...
public class Frag_EnergyWork extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		main_arr = getResources().getStringArray(R.array.EnergyWork_list);
		...
		filterText.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable arg0) {
				Frag_EnergyWork.this.arrAdapt.getFilter().filter(arg0);
	...
	public void myCalculate(String userValue, String Des) {
		EnergyWork newCal = new EnergyWork();
		...
		try {
			if (Des.equalsIgnoreCase("from")) {
				tempText = (newCal.EW(droppedItemFrom, droppedItemTo,
						enteredValue));

				valueTo.setText(userFormat(tempText));
				
			} else {
				tempText = (newCal.EW(droppedItemTo, droppedItemFrom,
						enteredValue));
