...
import infinity.zone.convertors.DataTransferRate;
...
public class Frag_DataTransferRate extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		main_arr = getResources().getStringArray(R.array.DTR_list);
		...
		filterText.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable arg0) {
				Frag_DataTransferRate.this.arrAdapt.getFilter().filter(arg0);
	...
	public void myCalculate(String userValue, String Des) {
		DataTransferRate newCal = new DataTransferRate();
		...
		try {
			if (Des.equalsIgnoreCase("from")) {
				tempText = (newCal.Dtr(droppedItemFrom, droppedItemTo,
						enteredValue));

				valueTo.setText(userFormat(tempText));
				
			} else {
				tempText = (newCal.Dtr(droppedItemTo, droppedItemFrom,
						enteredValue));
