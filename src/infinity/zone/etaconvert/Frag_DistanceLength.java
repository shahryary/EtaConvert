...
import infinity.zone.convertors.DistanceLength;
...
public class Frag_DistanceLength extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		main_arr = getResources().getStringArray(R.array.DL_list);
		...
		filterText.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable arg0) {
				Frag_DistanceLength.this.arrAdapt.getFilter().filter(arg0);
	...
	public void myCalculate(String userValue, String Des) {
		DistanceLength newCal = new DistanceLength();
		...
		try {
			if (Des.equalsIgnoreCase("from")) {
				tempText = (newCal.DL(droppedItemFrom, droppedItemTo,
						enteredValue));

				valueTo.setText(userFormat(tempText));
				
			} else {
				tempText = (newCal.DL(droppedItemTo, droppedItemFrom,
						enteredValue));
