...
import infinity.zone.convertors.Density;
...
public class Frag_Density extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		main_arr = getResources().getStringArray(R.array.Density_list);
		...
		filterText.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable arg0) {
				Frag_Density.this.arrAdapt.getFilter().filter(arg0);
	...
	public void myCalculate(String userValue, String Des) {
		...
		try {
			if (Des.equalsIgnoreCase("from")) {
				tempText = (newCal.Density(droppedItemFrom, droppedItemTo,
						enteredValue));

				valueTo.setText(userFormat(tempText));
				
			} else {
				tempText = (newCal.Density(droppedItemTo, droppedItemFrom,
						enteredValue));
