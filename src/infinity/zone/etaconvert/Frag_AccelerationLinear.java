...
import infinity.zone.convertors.AccelerationLinear;
...
public class Frag_AccelerationLinear extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		main_arr = getResources().getStringArray(R.array.AL_list);
		lv_arr = new ArrayList<String>(main_arr.length);
		for (String myArray : main_arr) {
			lv_arr.add(myArray);
		}
		arr = lv_arr.toArray(new String[lv_arr.size()]);
		arrAdapt = new ArrayAdapter<String>(getActivity(), R.layout.mylist, arr);
		...
		filterText.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable arg0) {
				Frag_AccelerationLinear.this.arrAdapt.getFilter().filter(arg0);
		...
		myList.setOnItemLongClickListener(new OnItemLongClickListener() {
			...
			public boolean onItemLongClick(AdapterView<?> l, View v,
				...
				v.startDrag(dragData, myShadow, arrAdapt.getItem(position), 0);
		...
		valueFrom.setOnFocusChangeListener(new OnFocusChangeListener() {
			...
			public void onFocusChange(View v, boolean hasFocus) {
				// custom dialog
				dialog = new CustomCalcDialog(v.getContext(), "");
	...
	public void myCalculate(String userValue, String Des) {
		AccelerationLinear newCal = new AccelerationLinear();
		...
		try {
			if (Des.equalsIgnoreCase("from")) {
				tempText = (newCal.AL(droppedItemFrom, droppedItemTo,
						enteredValue));

				valueTo.setText(userFormat(tempText));

			} else {
				tempText = (newCal.AL(droppedItemTo, droppedItemFrom,
						enteredValue));
