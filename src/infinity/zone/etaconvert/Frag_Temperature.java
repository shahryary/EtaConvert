...
import infinity.zone.convertors.Temperature;
...
public class Frag_Temperature extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		main_arr = getResources().getStringArray(R.array.Temp_list);
		...
		filterText.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable arg0) {
				Frag_Temperature.this.arrAdapt.getFilter().filter(arg0);
	...
	public void myCalculate(String userValue, String Des) {
		Temperature newCal = new Temperature();
		...
		try {
			if (Des.equalsIgnoreCase("from")) {
				tempText = (newCal.Temp(droppedItemFrom, droppedItemTo,
						enteredValue));

				valueTo.setText(userFormat(tempText));
				
			} else {
				tempText = (newCal.Temp(droppedItemTo, droppedItemFrom,
						enteredValue));
