...
import infinity.zone.convertors.Weight;
...
public class Frag_Weight extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		main_arr = getResources().getStringArray(R.array.Weight_list);
		...
		filterText.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable arg0) {
				Frag_Weight.this.arrAdapt.getFilter().filter(arg0);
	...
	public void myCalculate(String userValue, String Des) {
		...
		try {
			if (Des.equalsIgnoreCase("from")) {
				tempText = (newCal.Weight(droppedItemFrom, droppedItemTo,
						enteredValue));

				valueTo.setText(userFormat(tempText));
				
			} else {
				tempText = (newCal.Weight(droppedItemTo, droppedItemFrom,
						enteredValue));
