...
import infinity.zone.convertors.Speed;
...
public class Frag_Speed extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		main_arr = getResources().getStringArray(R.array.Speed_list);
		...
		filterText.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable arg0) {
				Frag_Speed.this.arrAdapt.getFilter().filter(arg0);
	...
	public void myCalculate(String userValue, String Des) {
		...
		try {
			if (Des.equalsIgnoreCase("from")) {
				tempText = (newCal.speed(droppedItemFrom, droppedItemTo,
						enteredValue));

				valueTo.setText(userFormat(tempText));
				
			} else {
				tempText = (newCal.speed(droppedItemTo, droppedItemFrom,
						enteredValue));
