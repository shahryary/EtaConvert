...
import infinity.zone.convertors.PowerCapacity;
...
public class Frag_PowerCapacity extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		main_arr = getResources().getStringArray(R.array.PC_list);
		...
		filterText.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable arg0) {
				Frag_PowerCapacity.this.arrAdapt.getFilter().filter(arg0);
	...
	public void myCalculate(String userValue, String Des) {
		PowerCapacity newCal = new PowerCapacity();
		...
		try {
			if (Des.equalsIgnoreCase("from")) {
				tempText = (newCal.PC(droppedItemFrom, droppedItemTo,
						enteredValue));

				valueTo.setText(userFormat(tempText));
				
			} else {
				tempText = (newCal.PC(droppedItemTo, droppedItemFrom,
						enteredValue));
