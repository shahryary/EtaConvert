...
import infinity.zone.convertors.Volume;
...
public class Frag_Volume extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		main_arr = getResources().getStringArray(R.array.Volume_list);
		...
		filterText.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable arg0) {
				Frag_Volume.this.arrAdapt.getFilter().filter(arg0);
	...
	public void myCalculate(String userValue, String Des) {
		...
		try {
			if (Des.equalsIgnoreCase("from")) {
				tempText = (newCal.Volume(droppedItemFrom, droppedItemTo,
						enteredValue));

				valueTo.setText(userFormat(tempText));
				
			} else {
				tempText = (newCal.Volume(droppedItemTo, droppedItemFrom,
						enteredValue));
